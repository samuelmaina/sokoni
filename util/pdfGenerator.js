...
module.exports = function createInvoice(invoice, path,name) {
...
function generateHeader(doc) {
  doc
    .image("public/logo/logo.jpg", 50, 45, {
      width: 50
    })
    .fillColor("#444444")
    .fontSize(20)
...
function generateCustomerInformation(doc, invoice,name) {
  doc
    ...
    .text(`Invoice Date: ${new Date()}`, 50, 215).moveDown(3)
    .text(`Purchaser : ${name}  `, 50, 240)
    .moveDown(2);
...
function generateInvoiceTable(doc, invoice) {
  ...
  doc.text(`Total Payment: Kshs ${invoice.total}`, 200, currentRowPosition+30,{align:'center'})
}